...
class TasksMarkdownParser:
    ...
    def _parse_phase(self, header: str, content: str) -> Dict:
    ...
    def _parse_module(self, header: str, content: str) -> Dict:
    ...
    def _parse_task(self, header: str, content: str) -> Dict:
        """Parse a single task."""
        ...
        dependencies = [dep.strip() for dep in dependencies_str.split(',') if dep.strip() and dep.strip().lower() != 'none']
    ...
    def _parse_unit_tests(self, content: str) -> Dict:
        ...
        # Parse code tests
        code_tests_match = re.search(r'- \*\*Code Tests:\*\*(.*?)(?=\n  - \*\*Visual Tests:\*\*|\Z)', tests_content, re.DOTALL)
        if code_tests_match:
    ...
    def _parse_test_items(self, content: str) -> List[Dict]:
        ...
        for test_name, test_description in test_matches:
            tests.append({
                ...
                "description": self._clean_text(test_description)
            })
...
def convert_md_to_json(md_file_path: str, json_file_path: str = None, indent: int = 2) -> str:
    """
    ...
    if json_file_path is None:
        ...
        json_file_path = md_path.parent / f"{md_path.stem}.json"
    
    # Parse the markdown file
    parser = TasksMarkdownParser()
    data = parser.parse_file(md_file_path)
    parser.save_to_json(json_file_path, indent)
...
def main():
    ...
    try:
        ...
        # Parse again to get summary (could be optimized by returning data from convert_md_to_json)
        parser_instance = TasksMarkdownParser()
        data = parser_instance.parse_file(args.input_file)
        
        # Print summary
        total_tasks = sum(len(module['tasks']) for phase in data['phases'] for module in phase['modules'])
        print(f"Summary: {len(data['phases'])} phases, "
